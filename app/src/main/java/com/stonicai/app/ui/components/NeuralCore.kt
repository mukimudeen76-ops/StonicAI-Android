...
fun NeuralCore(
    ...
    val rotation by transition.animateFloat(
        0f, 360f,
        infiniteRepeatable(tween(if (isSpeaking) 6000 else 18000, easing = LinearEasing)),
        label = "rot"
    )
    ...
    val nodeBounce by transition.animateFloat(
        0.85f, 1.15f,
        infiniteRepeatable(tween(1400), RepeatMode.Reverse), label = "nb"
    )
        ...
        Canvas(modifier = Modifier.size(size)) {
            ...
            drawCircle(
                brush = Brush.radialGradient(
                    colors = listOf(core.copy(alpha = 0.35f * pulse), core.copy(alpha = 0.10f * pulse), Color.Transparent),
            ...
            rotate(rotation, c) {
                for (i in 0 until 3) {
                    ...
                    drawOval(
                        ...
                        topLeft = Offset(c.x - r * scale, c.y - r * 0.85f),
            ...
            rotate(rotationReverse, c) {
                for (i in -2..2) {
                    ...
                    drawCircle(core.copy(alpha = 0.18f), radius = rr,
                        center = Offset(c.x, c.y + r * yFrac), style = Stroke(0.8f))
                }
            }
            drawCircle(
                brush = Brush.radialGradient(
            ...
            val orbitR = r * 0.78f
            for (i in 0 until 6) {
                val angle = Math.toRadians((360.0 / 6 * i + rotation))
                val nx = c.x + (orbitR * cos(angle)).toFloat()
                val ny = c.y + (orbitR * sin(angle)).toFloat()
                drawCircle(core, radius = 3.2f * if (i == 0) nodeBounce else 1f, center = Offset(nx, ny))
                drawCircle(core.copy(alpha = 0.5f), radius = 7f, center = Offset(nx, ny), style = Stroke(1f))
            }
        }
    }
}
