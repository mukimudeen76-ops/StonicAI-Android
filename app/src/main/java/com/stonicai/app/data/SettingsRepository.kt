...
data class StonicSettings(
    ...
    val effectiveSystemPrompt: String
        get() = systemPrompt.ifBlank { Persona.byId(personaId).systemPrompt }
}

object Models {
    data class Model(val id: String, val displayName: String, val provider: String, val apiModel: String)
...
class SettingsRepository(private val context: Context) {
    ...
    val settings: Flow<StonicSettings> = context.dataStore.data.map { p ->
        StonicSettings(
            ...
            ttsVolume = p[K.VOL] ?: 1.0f,
            ttsRate = p[K.RATE] ?: 1.05f,
            ttsPitch = p[K.PITCH] ?: 1.0f,
            ttsVoice = p[K.VOICE] ?: "",
            notifRequested = (p[K.NOTIF] ?: "0") == "1",
            keys = ApiKeys(
    ...
    suspend fun save(
        ...
        context.dataStore.edit { p ->
            ...
            ttsVoice?.let { p[K.VOICE] = it }
            notifRequested?.let { p[K.NOTIF] = if (it) "1" else "0" }
